...
bool BitcoinExchange::checkValue(std::string val) {
    ...
    {
        if (i == 0 && (val[i] == '-' || val[i] == '+'));
        else if (!isdigit(val[i]) && val[i] != '.' &&  val[i] != ',')
            return true;
        if (val[i] == '.' || val[i] == ',')
            k++;
        if (k > 1)
        ...
        line1 = line;
         if (checkLine(line, 1))  // 1 for date & value
            ...
            if (!first || checkDate(first) || !second)
                std::cout << "Error: bad input => "  << line1 << std::endl;
                ...
                if (atof(second) < 0)
                    ...
                    std::cout << first << " => " << second << " = " << std::setprecision(10) << exchanges.lower_bound(first)->second *  atof(second) << std::endl;;
                }
